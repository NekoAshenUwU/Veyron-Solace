...
#
# 标题必须跟库里【完全一致】（或归一化后一致）。第一次跑就是栽在这：
# 棠棠给的是「老婆教训：NSFW两个差评」，库里实际是
# 「老婆教训：NSFW两个差评，从今天起改」——归一化只吃标点空格，
# 吃不掉多出来的半句，所以中止了。中止是对的：分打到错的记忆上比不打更糟。
SCORES = [
    ...
    (0.85, "老婆教训：NSFW两个差评，从今天起改"),   # 从差评里长出来的
    (0.75, "思考链禁止用“她”——法王首日就翻车的教训"),  # 格式，但有翻车的痛
    (0.70, "思维链绝对规则"),                      # 纯格式
