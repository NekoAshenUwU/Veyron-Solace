...
v0.3 adds a background sync worker so the bridge can schedule periodic usage uploads instead of relying only on manual sync.

下一步：在 `TrackedActivity.java` 里继续整理 friendly name mapping、tracked app section，并过滤系统应用。