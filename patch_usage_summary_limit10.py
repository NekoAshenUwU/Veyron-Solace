#!/usr/bin/env python3
"""
棠予酿 MCP 工具层瘦身 —— 方案二 / limit 收紧(改成默认10)

只做:
  server.py 的 get_phone_usage_summary(): 默认 limit 从 5 改成 10。
  兼容两种起点: 如果还没打过上一版补丁(还是硬编码40),直接改成 limit=10；
  如果已经打过上一版(limit=5),把 5 改成 10。
不改其他任何函数、不改数据库。
可以安全重复运行。
"""
import shutil
...
NEW_LIMIT10 = '''@app.tool()
def get_phone_usage_summary(limit: int = 10) -> str:
    """获取手机今日使用总结，来自 CodeAndPurrs / Neko Usage Bridge。默认只拉最近10条，避免一次性占用过多上下文，需要更多可传 limit。"""
    rows = _neko_rows_from_codeandpurrs(limit)
    return json.dumps(_fmt_neko_usage_rows(rows), ensure_ascii=False, indent=2)'''
...
def main():
    ...
    if NEW_LIMIT10 in src:
        print("✅ 已经是 limit=10 了,跳过。")
        return

    if OLD_LIMIT5 in src:
        old = OLD_LIMIT5
        label = "limit=5 -> limit=10"
    elif OLD_ORIGINAL in src:
        old = OLD_ORIGINAL
        label = "硬编码40 -> limit=10"
    else:
        print("❌ 三种版本都没匹配到,结构跟预期不一样,中止,不动文件。")
        sys.exit(1)

    count = src.count(old)
    if count != 1:
        print(f"❌ [{label}] 匹配到 {count} 处(预期1处),中止,不动文件。")
        sys.exit(1)
    ...
    src = src.replace(old, NEW_LIMIT10)
